class Vec2 {
    ...
    rotateAroundPoint(point, rotation) {
        ...
        return new Vec2(point.x + dx * cosR - dy * sinR, point.y + dx * sinR + dy * cosR);
    }
}
...
function almostEquals(num1 = 1, num2 = 2, epsilon = 0.0005) {
...
class PgPhysicsObject {
    constructor(parent, positionOffset = Vec2.zero, isStatic = false, density, centerOfMass, cOR, staticFriction, dynamicFriction, type = "circle", width, height, radius, srcVertices, isRect, isPinned = false) {
        this.parent = parent;
    ...
    static createCircle(parent, positionOffset = Vec2.zero, radius, isStatic = false, density, centerOfMass, cOR, staticFriction, dynamicFriction, isPinned = false) {
    ...
    static createRectangle(parent, positionOffset = Vec2.zero, width, height, isStatic = false, density, centerOfMass, cOR, staticFriction, dynamicFriction, isPinned = false) {
    ...
    static createPolygon(parent, positionOffset = Vec2.zero, vertices, isStatic = false, density, centerOfMass, cOR, staticFriction, dynamicFriction, isPinned = false) {
...
class ObjectConstraint {
    ...
    update(dT) {
        ...
        if (almostEquals(currentLength, 0, 0.0001)) {
...
class PgPhysics {
            ...
            for (var i = 0; i < this.objects.length - 1; i++) {
                ...
                for (var j = i + 1; j < this.objects.length; j++) {
                    ...
                    if (colData.collided) {
                        ...
                        var colD = new CollisionData(obj1, obj2, colData.normal, colData.depth, Vec2.zero, Vec2.zero, 0);
    ...
    resolveCollision(cData = new CollisionData(), substep, substeps) {
        ...
        for (var i = 0; i < cData.contactCount; i++) {
            ...
            if (tangent.almostEquals(Vec2.zero, 0.0002)) {
                continue;
            }
    ...
    polygonToPolygonCollisionDetection(poly1, poly2, debug = false) {
        ...
        for (var i = 0; i < poly1.vertices.length; i++) {
            ...
            if (axisDepth_1 < depth) {
                if (debug == true) {
                    this.lineDebugList.push({ a: poly1.position, b: edge_1.divide(2).add(poly1.position), color: "#ffffff", offset: Vec2.zero });
                    this.lineDebugList.push({ a: poly1.position, b: axis_1.multiply(edge_1.length() / 2).add(poly1.position), color: "#ff0000", offset: Vec2.zero });
                }
                depth = axisDepth_1;
                normal = axis_1;
            }
        ...
        for (var i = 0; i < poly2.vertices.length; i++) {
            ...
            if (axisDepth_2 < depth) {
                if (debug == true) {
                    this.lineDebugList.push({ a: poly2.position, b: edge_2.divide(2).add(poly2.position), color: "#ffffff", offset: Vec2.zero });
                    this.lineDebugList.push({ a: poly2.position, b: axis_2.multiply(edge_2.length() / 2).add(poly2.position), color: "#ff0000", offset: Vec2.zero });
                }
                depth = axisDepth_2;
                normal = axis_2;
            }
