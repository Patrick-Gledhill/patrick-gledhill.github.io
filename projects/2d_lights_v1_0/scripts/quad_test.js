...
class SpotLight {
    constructor(x, y, radius, color, intensity, fov, rotation) {
        this.x = x;
        this.y = y;
        this.radius = radius;
        this.color = color;
...
function calculateLighting(lights, walls) {
    ...
    for (var light of lights) {
        ...
        for (var wall of walls) {
            if (wall instanceof WallCircle) {
                ...
                for (var i = 0; i < colMesh.vertices.length; i++) {
                    ...
                    quad.push(new Vertex2D(vertex1.x + (Math.cos(v1endv) * light.radius * shadowSize), vertex1.y + (Math.sin(v1endv) * light.radius * shadowSize)));

                    quad.push(new Vertex2D(vertex2.x + (Math.cos(v2endv) * light.radius * shadowSize), vertex2.y + (Math.sin(v2endv) * light.radius * shadowSize)));
                ...
                // var grad1 = ctx.createRadialGradient(light.x, light.y, 0, light.x, light.y, light.radius);
                // grad1.addColorStop(0, light.color);
        ...
        if (light instanceof SpotLight) {
            tmpCtx.translate(light.x, light.y);
            tmpCtx.rotate(-light.fov / 2);
            tmpCtx.rotate(light.rotation);
            tmpCtx.beginPath();
            tmpCtx.arc(0, 0, light.radius, 0, light.fov);
            tmpCtx.fill();
            tmpCtx.closePath();
        } else {
            tmpCtx.beginPath();
...
var lights = [
    new SpotLight(0, 0, 256, "#008000", 1, degreesToRadians(60), 0)
    // new Light(128, 0, 256, "#0080ff", 1)
...
createShape(0, 0, 32, 32, 5, "#0080ff", 0);
...
function main() {
    ...
    if (mouse.down == true) {
        if (selectedWall != null) {
            ...
            } else if (selectedWall instanceof PointLight || selectedWall instanceof SpotLight) {
                // selectedWall.x = mouse.x;
                // selectedWall.y = mouse.y;
