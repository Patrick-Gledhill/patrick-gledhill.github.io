...
class ParameterMaterial extends Material {
    ...
    scatter(rIn, hInfo) {
        ...
        var scatterDirection = hInfo.normal.add(vec3.randomUnitVector()).normalized();
        if (scatterDirection.nearZero()) {
            scatterDirection = hInfo.normal;
        }

        var specularDirection = vec3.reflect(rIn.direction(), hInfo.normal).normalized();
        ...
        var scatteredRay = new Ray(hInfo.p, rayDirection.normalized(), rIn.time());
...
class Metal extends Material {
    ...
    scatter(rIn, hInfo) {
        ...
        var scatteredRay = new Ray(hInfo.p, reflectedDirection.add(vec3.randomUnitVector().multiply(this.fuzz)).normalized(), rIn.time());
...
class Sphere extends HittableObj {
    constructor(center, radius, material, moving = false, center2) {
        super();
        this.center = center;
        this.center2 = moving == true ? center2 : center;
        this.radius = radius;
        this.material = material;
        this.isMoving = moving;
        var rvec = new vec3(this.radius, this.radius, this.radius);
        var box1 = new AABB(this.center.subtract(rvec), this.center.add(rvec));
        var box2 = new AABB(this.center2.subtract(rvec), this.center2.add(rvec));
